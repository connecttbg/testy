...
<div class="container container-narrow mb-4">
  ...
  <div class="text-center text-muted small mt-4">
    Aplikacja wykonana przez <a href="https://dataconnect.no" target="_blank" rel="noopener">dataconnect.no</a>
  </div>
</div>
</body>
</html>
...
def dashboard():
    if request.method == "POST":
        ...
        try:
            work_date_date = datetime.strptime(work_date_str, "%Y-%m-%d").date()
        except (TypeError, ValueError):
            flash("Nieprawidlowa data.")
            return redirect(url_for("dashboard"))
        now = datetime.now().date()
        if (now - work_date_date).days > 2:
            flash("Dodawac godziny pracy mozesz maksymalnie do 48H wstecz. Skontaktuj sie z Darkiem +4746572904")
            return redirect(url_for("dashboard"))
...
<div class="card p-3">
  ...
  <p class="text-muted small mb-2">
    Plik bazy danych: <code>{{ db_path }}</code><br>
...
""", files=files, db_path=DB_FILE, app_version=APP_VERSION)
